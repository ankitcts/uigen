...
export function extractComponentProps(source: string, filePath: string): ComponentPropsInfo[] {
  ...
  for (const pattern of patterns) {
    ...
    while ((match = pattern.exec(source)) !== null) {
      ...
      if (props.length > 0) {
        results.push({ componentName, filePath, props });
      }
    }
  }
...
function parsePropsString(raw: string): ComponentProp[] {
  ...
  for (let i = 0; i < raw.length; i++) {
    ...
    if (ch === "," && depth === 0) {
      ...
      if (trimmed) props.push(parseSingleProp(trimmed));
      current = "";
      continue;
    }
...
function parseSingleProp(propStr: string): ComponentProp {
  const eqIndex = propStr.indexOf("=");
  if (eqIndex !== -1) {
    return {
      name: propStr.substring(0, eqIndex).trim(),
      defaultValue: propStr.substring(eqIndex + 1).trim(),
    };
  }
  return { name: propStr };
}
...
export function createImportMap(files: Map<string, string>): ImportMapResult {
  ...
  const componentProps: ComponentPropsInfo[] = [];
  ...
  for (const [path, content] of files) {
    if (
      ...
      // Extract component props from source
      const fileProps = extractComponentProps(content, path);
      componentProps.push(...fileProps);
      ...
      if (missingImports) {
        missingImports.forEach((imp) => {
          ...
          if (isPackage) {
            ...
            imports[imp] = `https://esm.sh/${imp}?external=react,react-dom`;
          } else {
  ...
  for (const importPath of allImports) {
    ...
    if (isPackage) {
      ...
      const packageUrl = `https://esm.sh/${importPath}?external=react,react-dom`;
...
<body>
  ...
  ${componentProps.length > 0 ? `
    <div style="border-top: 1px solid #e5e7eb; background: #f9fafb; padding: 16px 24px; font-size: 13px;">
      <div style="font-weight: 600; font-size: 12px; text-transform: uppercase; letter-spacing: 0.05em; color: #6b7280; margin-bottom: 12px;">Props</div>
      ${componentProps.map(comp => `
        <div style="margin-bottom: 12px;">
          <div style="font-weight: 600; color: #111827; margin-bottom: 6px;">${comp.componentName} <span style="font-weight: 400; color: #9ca3af; font-size: 12px;">${comp.filePath}</span></div>
          <table style="width: 100%; border-collapse: collapse;">
            <thead>
              <tr style="text-align: left; font-size: 11px; text-transform: uppercase; letter-spacing: 0.05em; color: #9ca3af;">
                <th style="padding: 4px 8px; border-bottom: 1px solid #e5e7eb; font-weight: 500;">Name</th>
                <th style="padding: 4px 8px; border-bottom: 1px solid #e5e7eb; font-weight: 500;">Default</th>
              </tr>
            </thead>
            <tbody>
              ${comp.props.map(p => `
                <tr>
                  <td style="padding: 4px 8px; border-bottom: 1px solid #f3f4f6; font-family: 'SF Mono', Monaco, Consolas, monospace; color: #7c3aed; font-size: 13px;">${p.name}</td>
                  <td style="padding: 4px 8px; border-bottom: 1px solid #f3f4f6; font-family: 'SF Mono', Monaco, Consolas, monospace; color: #059669; font-size: 13px;">${p.defaultValue ? p.defaultValue.replace(/</g, '&lt;').replace(/>/g, '&gt;') : '<span style="color: #d1d5db;">\u2014</span>'}</td>
                </tr>
              `).join('')}
            </tbody>
          </table>
